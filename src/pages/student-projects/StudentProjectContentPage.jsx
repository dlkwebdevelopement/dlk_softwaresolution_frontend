...
export default function StudentProjectContentPage() {
  ...
  useEffect(() => {
    ...
    const fetchProject = async () => {
      try {
        setLoading(true);
        const res = await GetRequest(ADMIN_GET_STUDENT_PROJECTS_SLUG(slug));
  ...
  if (!project) return (
    <Box sx={{ py: 20, textAlign: 'center' }}>
      <Typography variant="h5" fontWeight={600}>Project not found</Typography>
      <Typography 
        onClick={() => navigate('/')}
            ...
            >
              {latestPosts.map((post, index) => (
                <Card 
                  ...
                  onClick={() => navigate(`/student-projects/${post.slug}`)}
                  sx={{ 
